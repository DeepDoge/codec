...
- **Default endianness changed**: Previously the default was little-endian (LE),
  ...
  encoding.
- **VarInt API changed**: Removed `encodeVarInt` and `decodeVarInt` exports. Use
  the `varint` codec instead:
  ```typescript
...
```typescript
import { str, StructCodec, u32, VectorCodec } from "@nomadshiba/codec";
...
```typescript
import { str, StructCodec, u32 } from "@nomadshiba/codec";
...
```typescript
import { u16, u32, VectorCodec } from "@nomadshiba/codec";
...
```typescript
import { MappingCodec, str, u32, u8 } from "@nomadshiba/codec";
